...
{
public:
    TCS3471(void (*i2cWriteFunc)(uint8_t,uint8_t,uint8_t*),void (*i2cReadFunc)(uint8_t,uint8_t,uint8_t*));
    bool detect();
    bool enable();
    void disable();
    ...
    void clearInterrupt();
    void interruptHighThreshold(uint16_t highThreshold);
    void interruptLowThreshold(uint16_t lowThreshold);
    void interruptPersistence(uint8_t persistence);
    uint8_t getChipID();
    bool rgbcValid();
    uint16_t readCData();
    uint16_t readRData();
...
private:
    ...
    void (*_i2cWrite)(uint8_t,uint8_t,uint8_t*);
    void (*_i2cRead)(uint8_t,uint8_t,uint8_t*);
};
